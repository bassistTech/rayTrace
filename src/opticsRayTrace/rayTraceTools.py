...
def build_geometry(surface_list):
    ...
    # Allow plotting routines to use the Matplotlib color cycle
    ...
    for i in range(len(surface_list)):
        ...
        g["basis"] = basis
        # Set the origin of the next surface, distance is measured along local Z axis
        origin = origin + g["dist"] * basis[2]
        geometry.append(g)
...
help_ray_table = """
    ...
    type = 4
        data = intersection in local coordinates of surface
    """
...
default_aim_consts = np.zeros((6))

def new_ray_table(geometry, fields, pupils, wavls, infinite,
                  aim_consts=default_aim_consts):
    """
    ...
    ray_table = np.zeros([len(geometry), nrays, 5, 3])
    ...
    for j in range(len(fields)):
        ...
        for k in range(len(pupils)):
            ...
            for m in range(len(wavls)):
                ...
                ray_table[0, i, 4, :] = ray_table[0, i, 0, :]
                i = i + 1
...
def propagate_conic_surface(ray_table, geometry, surf):
    ...
    # pre-declare arrays to avoid re-declaring inside the loop
    sag = np.zeros([ray_table.shape[1]])
    sagnew = np.zeros([ray_table.shape[1]])
    r = np.empty_like(sagnew)
    for k in range(40):
        ray_table[surf, :, 0, :] = intersect_line_plane_array(
            surface["origin"] + np.outer(sag, surface["basis"][2]), 
            surface["basis"][2],
    ...
    ray_table[surf, :, 4, 2] = sag
    ray_table[surf, :, 4, 0] = np.dot(ray_table[surf, :, 0, :] - surface["origin"], surface["basis"][0])
    ray_table[surf, :, 4, 1] = np.dot(ray_table[surf, :, 0, :] - surface["origin"], surface["basis"][1])
    n = normalize_vec_array(np.outer(sd*ray_table[surf, :, 4, 0], surface["basis"][0])
                            + np.outer(sd*ray_table[surf, :, 4, 1], surface["basis"][1])
                            - surface["basis"][2])
...
def propagate_cylindrical_surface(ray_table, geometry, surf):
    ...
    for k in range(40):
        ray_table[surf, :, 0, :] = intersect_line_plane_array(
            surface["origin"] +
            np.outer(sag, surface["basis"][2]), surface["basis"][2],
            ray_table[surf-1, :, 0, :], ray_table[surf-1, :, 1, :])
        ya = np.dot(ray_table[surf, :, 0, :] -
                    surface["origin"], surface["basis"][1])
    ...
    ray_table[surf, :, 4, 0] = np.dot(ray_table[surf, :, 0, :] - surface["origin"], surface["basis"][0])
    ray_table[surf, :, 4, 1] = np.dot(ray_table[surf, :, 0, :] - surface["origin"], surface["basis"][1])
    n = normalize_vec_array(0*np.outer(sd*ray_table[surf, :, 4, 0], surface["basis"][0])
                            + np.outer(sd*ray_table[surf, :, 4, 1], surface["basis"][1])
                            - surface["basis"][2])
...
def propagate_plane_grating(ray_table, geometry, surf):
    ...
    exy = (
        n1 * np.dot(ray_table[en, :, 1, :], surface["basis"][1])
...
def propagate_plane_grating_2(ray_table, geometry, surf):
    ...
    exx = (
        n1 * np.dot(ray_table[en, :, 1, :], surface["basis"][0])
        + surface["m"] * wavl / surface["d"]
    ) / n2
...
glass_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'glass_list.pickle')
glass_list = pickle.load(open(glass_filename, 'rb'))
