...
def new_ray_table(geometry, field_points, pupil_points, wavls):
    ...
    # ray_table = [surf, ray, type, p (type=0), v (type=1), [n1 n2 wn] (type=2)]
    ray_table = np.empty([len(geometry), nrays, 3, 3])
    mg = np.array(np.meshgrid(range(len(field_points)), range(
        len(pupil_points)), range(len(wavls)))).T.reshape(-1, 3)
...
def new_ray_table_infinite(geometry, field_rays, pupil_points, wavls):
    nrays = len(field_rays)*len(pupil_points)*len(wavls)
    # ray_table = [surf, ray, type, p (type=0), v (type=1), [n w _] (type=2)]
    ray_table = np.empty([len(geometry), nrays, 3, 3])
    mg = np.array(np.meshgrid(range(len(field_rays)), range(
        len(pupil_points)), range(len(wavls)))).T.reshape(-1, 3)
    # https://stackoverflow.com/questions/1208118/using-numpy-to-build-an-array-of-all-combinations-of-two-arrays
    ...
    for i in range(ray_table.shape[1]):  # i = ray
        j, k, m = mg[i]  # j = field, k = pupil, m = wavelength
        ray_table[0, i, 1, :] = normalize_vec(field_rays[j])
        ray_table[0, i, 0, :] = pupil_points[k]
        ray_table[0, i, 2, 1] = wavls[m]
